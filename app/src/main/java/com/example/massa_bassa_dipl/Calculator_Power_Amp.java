...
public class Calculator_Power_Amp extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Amp Power Calculator");
    ...
    private void AmpResult() {
        // Get inputs
        int numSpeakers =
                Integer.parseInt(numSpeakersEditText.getText().toString());
        int rms = Integer.parseInt(rmsEditText.getText().toString());
        int maxPower =
                Integer.parseInt(maxPowerEditText.getText().toString());
        ...
        double recommendedRmsPower = calculateRecommendedRmsPower(numSpeakers,
                rms, ohms, isDvc);
        double recommendedMaxPower =
                calculateRecommendedMaxPower(recommendedRmsPower);
        // Construct output string
        String output = "Recommended minimum RMS power: \n" +
                roundToTwoDecimals(recommendedRmsPower) + " watts\n"
                + "Recommended maximum RMS power: \n" +
                roundToTwoDecimals(recommendedMaxPower) + " watts\n"
                + "Recommended amplifier: \n" +
                getRecommendedAmplifier(recommendedRmsPower, recommendedMaxPower, isDvc);
        // Display output
        outputAmpTextView.setText(output);
    }
    private double calculateRecommendedRmsPower(int numSpeakers, int rms, int
            ohms, boolean isDvc) {
    ...
    private String getRecommendedAmplifier(double recommendedRmsPower, double
            recommendedMaxPower, boolean isDvc) {
        String recommendedAmplifier = recommendedRmsPower + "/" +
                recommendedMaxPower + " watts";
